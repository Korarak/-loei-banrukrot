...
export default function CustomerLoginPage() {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        try {
            ...
            // บัญชีที่สมัครผ่าน Google ไม่มีรหัสผ่าน — ชี้ไปที่ปุ่ม Google ด้านล่างแทนข้อความ error เฉยๆ
            if (error.response?.data?.code === 'ACCOUNT_USES_GOOGLE') {
                toast.error('บัญชีนี้สมัครผ่าน Google', {
                    description: 'กรุณาเข้าสู่ระบบด้วยปุ่ม Google ด้านล่าง',
                    duration: 6000,
                });
            } else {
        ...
        <div className="container mx-auto flex items-center justify-center min-h-[calc(100vh-4rem)] p-4 relative">
            <Card className="w-full max-w-md relative z-10">
                <CardHeader className="space-y-2 pt-8">
                    <div className="font-display uppercase text-3xl text-center text-gray-900 leading-none">Sign In</div>
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input type="password" autoComplete="current-password" placeholder="••••••" className="h-12 font-medium" {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
